...
class Levels(commands.Cog):
    ...
    async def lvl_up(self, user):
        ...
        if cur_exp >= round((15 + 3 * (cur_lvl ** 4)) / 5):
    ...
    async def on_message(self, message):
        ...
        await self.bot.pg_con.execute("UPDATE users SET xp = $1 WHERE user_id = $2 AND guild_id = $3", user['xp'] + 2, author_id, guild_id)
    ...
    async def level(self, ctx, member: discord.Member = None):
        ...
        if not user:
            ...
            embed.add_field(name="Credits", value=user[0]['Credits'])
    ...
    async def credit(self, message):
        ...
        if not user:
            await self.bot.pg_con.execute("INSERT INTO users (user_id, guild_id, lvl, xp) VALUES ($1, $2, 1, 0)",
                                          author_id, guild_id)
        ...
        await self.bot.pg_con.execute("UPDATE users SET xp = $1 WHERE user_id = $2 AND guild_id = $3", user['Credits'] + 2,
                                      author_id, guild_id)
